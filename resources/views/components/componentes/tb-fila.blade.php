...
<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
    ...
    @if (isset($fila->ap_paterno))
        <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
            ...
            <div class="ps-3">
                <!-- Nombre -->
                <div class="text-base font-semibold">{{ $fila->nombre}} {{ $fila->ap_paterno}} {{ $fila->ap_materno}}</div>
                <!-- Correo -->
                <div class="font-normal text-gray-500">{{ $fila->correo }}</div>
            </div>
        </th>
        @php
    ...
    @foreach ($datos as $datos )
        ...
        @php
            ...
            $datos = str_replace('de', '_', $datos);
            $datos = str_replace('___', '_', $datos);
            $datos = strtolower($datos);
        @endphp

        <td class="px-6 py-4">
            @if($datos == 'estado')
                @if($fila->estado_usuario)
                    <div class="flex items-center">
