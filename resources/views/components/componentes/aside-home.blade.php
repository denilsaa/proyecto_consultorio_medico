...
'subitems' => [
['uri' => route('personal.index'), 'titulo' => 'Personal'],
['uri' => route('pacientes.index'), 'titulo' => 'Pacientes']
]
],
/* [
'uri' => route('roles.index'),
'titulo' => 'Roles',
...
'subitems' => [
['uri' => route('farmacos.index'), 'titulo' => 'Farmaco'],
['uri' => route('presentaciones.index'), 'titulo' => 'Presentacion']
]
],
[
'uri' => route('citas.index'),
'titulo' => 'Citas',
'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard-pulse" viewBox="0 0 16 16">
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-sidebar border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto  dark:bg-gray-800">
        <ul class="space-y-2 font-medium">
            @foreach ($items as $item)
            <x-componentes.item-aside-home :item="$item">
                {!! $item['icon'] !!}
            </x-componentes.item-aside-home>
            @endforeach
        </ul>
        <ul class="pt-4 mt-4 space-y-2 font-medium border-t border-gray-200 dark:border-gray-700">
            @foreach ($itemsDos as $item)
            <x-componentes.item-aside-home :item="$item">
                {!! $item['icon'] !!}
            </x-componentes.item-aside-home>
            @endforeach
            ...
            <label class="inline-flex items-center me-5 cursor-pointer">
                <input type="checkbox" value="" class="sr-only peer" id="dark-mode-toggle" inert>
                ...
                <span class="flex items-center p-2 rounded-lg text-white hover:bg-gray-700 transition-all duration-200 group hover:shadow-xl">Dark</span>
            </label>
        </ul>
    </div>
</aside>
